...
Per quanto riguarda le altre zone termiche (\corpa\ e \corpc) l'impianto utilizzato è di tipo \emph{misto} (come nel III e IV piano) o addirittura assente. Pertanto non è stata effettuata una valutazione pre intervento in quanto sarebbe risultato difficile oltre che inutile. 

Le portate di ventilazione per il \corpa\ e \textbf{C} sono state calcolate definendo di volta in volta la destinazione d'uso per ogni locale: utilizzando l'affollamento o i ricambi orari a seconda delle esigenze (utilizzando la \norvent).
...
Per determinare le condizioni termoigrometriche di immissione dell'aria nell'UTIC, Emodinamica e Blocco Operatorio si è proceduto facendo un bilancio di energia:
\begin{align}
...
Dalla conoscenza della $t_s$ si è calcolata la potenza necessaria per portare l'aria esterna alle condizioni desiderate. Nella seguente tabella sono riassunti i risultati.
\begin{center}
	\label{UTA:potenzaFATTO}
	\begin{tabular}{lcc}
		
					&	$t_s$ [\si{\degreeCelsius}]				&	kW	\\
		\midrule
		UTIC				&	\num{23.3}						&	\num{33.9}\\
		Emod.				&	\num{27.4}						&	\num{46.2}\\
		B.O.				&	\num{25.2}						&	\num{185}\\
	\end{tabular}
\end{center}
In definitiva, i risultati sono riportati di seguito per ogni zona termica. CONTROLLA I VALORI DI QUESTA TABELLA CON QUELLI DI EXCEL, AGGIORNA I VALORI DEL CORPO A/C SU EXCEL E AGGIORNA QUA
\begin{center}
	\small
	\begin{tabular}{lcccc}
		\toprule
